...
"""Проверим, что во всех магазинах содержатся товары (город по умолчанию)
    и найденные товары соответствуют поисковому запросу, для каждого маркетплейса
    по отдельности, начиная с последнего в списке навигационной панели"""


@pytest.mark.parametrize('word', ['iphone 12', 'купальник Mad Wave'])
def test_search_kazan(browser, word):
    """Проверим, что найденные в KazanExpress товары соответствуют
    поисковому запросу"""
    main_page = MainPageSearchHelper(browser)
    search_page = SearchPageHelper(browser)
    main_page.go_to_site()
    # main_page.choice_city().click()

    # words = main_page.choice_search_word()
...
def test_check_sort_by(browser):
    """Проверим, что популярность использует только отзывы"""
    main_page = MainPageSearchHelper(browser)
    search_page = SearchPageHelper(browser)
    main_page.go_to_site()

    main_page.enter_word('iphone 12')
    main_page.click_on_search_button()
    browser.implicitly_wait(10)
    pops_all = search_page.prods_pops_all_list()
    for pops in pops_all:
        msg = 'Wrong pops used'
        assert 'отзывы' in pops.split()[1]
...
def test_check_sort_by_pops(browser):
    ...
    main_page.enter_word('смартфон iphone 12')
    main_page.click_on_search_button()
    browser.implicitly_wait(10)
